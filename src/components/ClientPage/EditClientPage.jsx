    ...
    try {
      ...
      if (response.data.success) {
        ...
        reset({
          ...
          street_no: clientData.street_no,
          city: clientData.city,
          state: { label: clientData.state, value: clientData.state },
          country: { label: clientData.country, value: clientData.country },
          Pan_gst: clientData.pan_gst,
          contacts: clientData.other_contacts.map(contact => ({
            name: contact.name,
            email: contact.email,
        ...
        // Set the country and then update states
        const countryOption = getCountries().find(
          country => country.label === clientData.country
        );
        if (countryOption) {
          setValue("country", countryOption);
          setStates(getStates(countryOption.value));
        }
      } else {
        setErrorMessage("Failed to fetch client data");
      }
    } catch (error) {
      setErrorMessage("Error fetching client data");
    }
    setIsLoading(false);
  };
  ...
  const onSubmit = async data => {
    ...
    try {
      const formattedData = {
        ...
        other_contacts: data.contacts.map(contact => ({
          name: contact.name,
