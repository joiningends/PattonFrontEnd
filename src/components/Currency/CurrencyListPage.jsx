...
export default function CurrencyListPage() {
  ...
  const fetchCurrency = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setError("Failed to fetch currency data" || response.data.message);
      }
    } catch (error) {
      setError(
  ...
  const filteredCurrency = currency.filter(curr =>
    curr.currency_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    curr.currency_code.toLowerCase().includes(searchTerm.toLowerCase()) ||
    curr.currency_value.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const indexOfLastCurrency = currentPage * currencyPerPage;
  const indexOfFirstCurrency = indexOfLastCurrency - currencyPerPage;
  const currentCurrency = filteredCurrency.slice(
    indexOfFirstCurrency,
    indexOfLastCurrency
  );
  const totalPages = Math.ceil(
    filteredCurrency.length / currencyPerPage
  );
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this currency?")) {
      try {
        ...
        if (response.data.success) {
          fetchCurrency();
          setSuccessMessage("Currency deleted successfully" || response.data.message);
          setTimeout(() => {}, 1000);
        } else {
          setError("Failed to delete currency data. Please try again.");
        }
      } catch (error) {
        setError(
          "Error deleting currency data: " +
          (error.response?.data?.message || error.message)
        );
      }
    }
          ...
          >
            <div>
              ...
              <p className="text-[#4b4b80] text-base lg:text-lg">
                Manage your currency conversion
              </p>
            </div>
              ...
              <table className="w-full text-sm text-left text-[#2d2d5f]">
                <thead className="text-xs uppercase bg-gradient-to-r from-[#000060] to-[#0000a0] text-white">
                  <tr>
                    ...
                    <th className="px-6 py-4 font-extrabold text-sm">
                      <div className="flex items-center">Currency Code</div>
                    </th>
                    {/* <th className="px-6 py-4 font-extrabold text-sm">
                      <div className="flex items-center">
                        <Package className="mr-2" />
                  ...
                  {currentCurrency.map((curr, index) => (
                    ...
                    >
                      <td className="px-6 py-4 font-medium">
                        <div className="flex items-center">
                          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#000060] to-[#0000a0] text-white flex items-center justify-center mr-3 text-sm font-semibold shadow-md">
                            {curr.currency_name.charAt(0).toUpperCase()}
                          </div>
                      ...
                      <td className="px-6 py-4">
                        {!isNaN(parseFloat(curr?.currency_value))
                          ? parseFloat(curr.currency_value).toFixed(2)
                          : "N/A"}
                      </td>
                      <td className="px-6 py-4">
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-semibold ${curr.status
