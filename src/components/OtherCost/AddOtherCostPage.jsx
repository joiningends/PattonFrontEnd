...
export default function CreateOtherCostPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axiosInstance.post("/other-cost/save", costData);
            if (response.data.success) {
                setSuccessMessage("Other cost created successfully");
                setTimeout(() => {
                    navigate("/other-costs");
                }, 2000);
            } else {
                setErrors({
                    submit: response.data.message || "Failed to create other cost",
                });
            }
        } catch (error) {
            setErrors({
                submit: error.response?.data?.message || "Error creating other cost",
            });
        }
            ...
            <div className="max-w-6xl mx-auto">
                ...
                >
                    ...
                    <p className="text-[#4b4b80] text-base lg:text-lg">
                        Add a new cost type to the system
                    </p>
                </motion.div>
                ...
                >
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="flex justify-end space-x-4 pt-6">
                            ...
                            >
                                {isSubmitting ? "Creating..." : "Create Other Cost"}
                            </button>
                        </div>
                    </form>
                </motion.div>
