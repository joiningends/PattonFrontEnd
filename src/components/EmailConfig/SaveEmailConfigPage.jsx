...
import { ArrowLeft, DollarSign, X, Info } from "lucide-react";
import axiosInstance from "../../axiosConfig";

export default function SaveEmailConfigPage() {
    ...
    const validateForm = () => {
        ...
        } else if (emailConfigData.pass.length !== 19) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axiosInstance.post("/email-config/save", emailConfigData);
            if (response.data.success) {
                setSuccessMessage(response.data.message || "Email config created successfully");
                setTimeout(() => {
                    navigate("/email-config");
                }, 2000);
            } else {
                setErrors({
                    submit: response?.data?.message || "Failed to create email config",
                });
            }
        } catch (error) {
            setErrors({
                submit: error.response?.data?.message || "Error creating email config",
            });
        }
            ...
            <div className="max-w-6xl mx-auto">
                ...
                >
                    ...
                    <p className="text-[#4b4b80] text-base lg:text-lg">
                        Add a new email config type to the system for notification
                    </p>
                </motion.div>
                ...
                >
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="flex justify-end space-x-4 pt-6">
                            ...
                            >
                                {isSubmitting ? "Creating..." : "Create Email Config"}
                            </button>
                        </div>
                    </form>
                </motion.div>
